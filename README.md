...
| **보너스 2** | **UX 고도화 & 방문자 분석 (Analytics)** | • **실시간 방문자 & 풀이 통계 위젯**: 당일 방문자 수, 누적 운세 풀이 수 실시간 집계 및 카운팅 애니메이션<br>• **사용자 이벤트 트래커 (`trackEvent`)**: 세션별 섹션 이동, 풀이 요청, 공유 등 행동 로그 추적<br>• **마이크로 인터랙션**: FAQ `+` 45도 회전 아코디언, 로딩 펄스 & 3.5초 지연 안내 문구 자동 전환, 플로팅 토스트, 글래스 카드 호버 리프트 |
